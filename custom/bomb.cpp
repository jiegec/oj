    ...
    if (dir < 4) {
      ...
      if (dir == NW) {
        return dfs3(x + d[N][0], y + d[N][1], N) &&
               dfs3(x + d[W][0], y + d[W][1], W) &&
               dfs3(x + d[NW][0], y + d[NW][1], NW);
      } else if (dir == NE) {
        return dfs3(x + d[N][0], y + d[N][1], N) &&
               dfs3(x + d[E][0], y + d[E][1], E) &&
               dfs3(x + d[NE][0], y + d[NE][1], NE);
      } else if (dir == SE) {
        return dfs3(x + d[S][0], y + d[S][1], S) &&
               dfs3(x + d[E][0], y + d[E][1], E) &&
               dfs3(x + d[SE][0], y + d[SE][1], SE);
      } else if (dir == SW) {
        return dfs3(x + d[S][0], y + d[S][1], S) &&
               dfs3(x + d[W][0], y + d[W][1], W) &&
               dfs3(x + d[SW][0], y + d[SW][1], SW);
      }
    }
  case 'W':
    map[x][y] = '-';
...
bool dfs(int bomb, int x, int y) {
  if (bomb == n) {
    for (int i = 0; i < r; i++) {
      for (int j = 0; j < c; j++) {
        if (map[i][j] == '*' || map[i][j] == 'W' || map[i][j] == 'N' ||
            map[i][j] == 'E' || map[i][j] == 'S')
          return false;
      }
    }
