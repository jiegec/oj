...
public:
  ...
  BigNumber pre(BigNumber index) {
    LinkNode *current = head;
    if (current == NULL || (current->index > index || current->index == index)) {
      return ZERO;
    }
    BigNumber sum;
    while (current != NULL) {
      sum = sum + ONE;
      if ((current->next && (current->next->index > index || current->next->index == index)) || current->next == NULL) {
        return sum;
      }
      current = current->next;
    }
  ...
  void set(BigNumber index, int n) {
    ...
    if (current == NULL) {
      ...
      while (current != NULL) {
        ...
        if ((current->next && current->next->index > index) || current->next == NULL) {
