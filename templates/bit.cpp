...
uint16_t rev(uint16_t x) {
  return (uint16_t)(refl(x) >> 16);
}

// de Bruijn cycle, by Danny Dube
uint32_t ntz_alt(uint32_t x) {
  static uint8_t table[32] = {0, 1, 2, 24, 3, 19, 6, 25, 22, 4, 20, 10, 16, 7, 12, 26, 31, 23, 18, 5, 21, 9, 15, 11, 30,
                              17, 8, 14, 29, 13, 28, 27};
  if (x == 0)
    return 32;
  x = (x & -x) * 0x04D7651F;
...
uint32_t ntz_alt2(uint32_t x) {
  static uint8_t table[37] = {32, 0, 1, 26, 2, 23, 27, 0, 3, 16, 24, 30, 28, 11, 0, 13, 4, 7, 17, 0, 25, 22, 31, 15, 29,
...
T toggle_least_1(T s) { return s & (s - 1); };

// 10100111 -> 10101111
template<typename T, u<T>>
T toggle_least_0(T s) { return s | (s + 1); };

// 10100111 -> 10100000
// can be used to check if s is 2^{n-1}, 0 or all 1
template<typename T, u<T>>
T toggle_trailing_1(T s) { return s & (s + 1); };

// 10101000 -> 10101111
template<typename T, u<T>>
T toggle_trailing_0(T s) { return s | (s - 1); };

// 01011000 -> 00001000
template<typename T, u<T>>
T extract_least_1(T s) { return s & (-s); }

// 10101000 -> 11110111
template<typename T, u<T>>
T extract_least_1_alt(T s) { return ~s | (s - 1); }

// 10101000 -> 00001111
template<typename T, u<T>>
T extract_least_1_alt2(T s) { return s ^ (s - 1); }

// 10100111 -> 00001000
template<typename T, u<T>>
T extract_least_0(T s) { return ~s & (s + 1); }

// 10100111 -> 00001111
template<typename T, u<T>>
T extract_least_0_alt(T s) { return s ^ (s + 1); }

// 01011000 -> 0000111
// or ~(s|-s) or (s&-s)-1
template<typename T, u<T>>
T extract_trailing_0(T s) { return ~s & (s - 1); }

// 10100111 -> 11111000
template<typename T, u<T>>
T extract_trailing_1(T s) { return ~s | (s + 1); }

// 01011100 -> 01000000
// can be used to check if s is 2^j-2^k
// or ((s&-s)+s)&s
template<typename T, u<T> = 1>
T toggle_trailing_contiguous_1(T s) { return ((s | (s - 1)) + 1) & s; }
...
template<typename T, u<T> = 1>
void round_down_to(T s, T t) {
  static_assert((t & (t - 1)) == 0, "t must be a power of 2");
  return s & (-t);
};

template<typename T, u<T> = 1>
void round_up_to(T s, T t) {
  static_assert((t & (t - 1)) == 0, "t must be a power of 2");
  return (s + t - 1) & (-t);
};
...
template<typename T, u<T> = 1>
bool cross_power_of_2(T a, T l, T p) {
  // or p-(a&(p-1))<l
  return -(a | -p) < l;
}
