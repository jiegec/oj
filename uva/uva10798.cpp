  ...
  while (scanf("%d\n",&n) && n) {
    ...
    while (!pq.empty()) {
      ...
      for (int i = 0;i < 4;i++) {
        ...
        temp.x += d[i][0];
        temp.y += d[i][1];
        if (map[temp.x][temp.y] == 'R') temp.up ++;
        if (map[n-1-temp.y][temp.x] == 'R') temp.left ++;
        if (map[n-1-temp.x][n-1-temp.y] == 'R') temp.down++;
        if (map[temp.y][n-1-temp.x] == 'R') temp.right++;
        ...
        if (!visit[temp.x][temp.y][temp.up][temp.right][temp.down][temp.left]) {
          visit[temp.x][temp.y][temp.up][temp.right][temp.down][temp.left] = true;
          pq.push(temp);
        }
      }
    }
